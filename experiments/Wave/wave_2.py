...
for i in range(k):
    up = [stream]
    for i in range(n):
        up.append(random_state.normal(0.2+0.1*(i/n), u, 1))
...
test_drift_detector = drift.atwin.ATWIN(delta=o, temp=1000)
test_drift_count = 0
test_drifts = [n] * d
...
for i, val in enumerate(stream):
    ...
    if test_drift_detector.drift_detected:
        ...
        test_drift_count += 1
        test_drifts[i // n] = min(test_drifts[i // n], i % n)
