...
class Model:
    ...
    def foward(self, x, y, i):
        ...
        if y == self.model.predict_one(x):
            self.acc.append(True)
        else:
            self.acc.append(False)
        if len(self.acc) > self.accL:
            self.acc.pop(0)

        e = self.acc.count(True) / len(self.acc)
        ...
        if self.detector:
            ...
            if self.detector.drift_detected:
                ...
                for _x, _y in self.retrain_set:
                    ...
                    if _y == self.model.predict_one(_x):
                        self.acc.append(True)
                    else:
                        self.acc.append(False)
                    if len(self.acc) > self.accL:
...
NBAtwin = Model(drift.atwin.ATWIN(delta=0.0005, temp=1/1000))
