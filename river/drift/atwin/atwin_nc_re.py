...
class AdaptiveWindowing:
                    ...
                    for k in range(bucket.current_idx - 1):
                        ...
                        if (
                            ...
                            if self.width > 0:
                                ...
                                exit_flag = True    # We are done
                                #plot_data([[raw_resized], [inweights], [outweights]])
                                break
    ...
    def _attent(self, d, raw):
        # all bucket totals
        q = self.query[d](torch.tensor([raw[-1]], dtype=torch.float64).float())
        k = self.key(torch.tensor(raw, dtype=torch.float64).float())
