...
from libc.math import fabs, log, sqrt
...
cdef class AdaptiveWindowing:
    ...
    cdef bint _detect_change(self):
        ...
        cdef:
            unsigned int idx, k
            bint change_detected, exit_flag, size, total
            double n0, n1, w0, w1, wu0, wu1
            Bucket bucket
        ...
        if (self.tick % self.clock == 0) and (self.width > self.grace_period):
            ...
            for i in range(len(self.bucket_deque)):
                ...
                for j in range(bucket.current_idx):
                    total = bucket.get_total_at(j)
                    raw.append(total)
                    raw_resized.append([total / size])
    ...
    cdef _context(self, raw):
        ...
        for i in range(self.d):
            ...
            k = self.key[i](torch.tensor(raw, dtype=torch.float64).float())
            ...
            if new[0] >= threshold:
                if inweights.any():
                    inweights = np.add(inweights, w)
                else:
                    inweights = w
            # out context
            else:
                if outweights.any():
                    outweights = np.add(outweights, w)
                else:
                    outweights = w
